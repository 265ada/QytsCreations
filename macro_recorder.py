...
import winsound, subprocess, ctypes, ctypes.wintypes, dataclasses
...
HOOK_DIR         = Path(__file__).resolve().parent / "hooks" / "dinput_hook"
HOOK_DLL_PATH    = HOOK_DIR / "dinput_hook.dll"
HOOK_INJECTOR    = HOOK_DIR / "injector.exe"


def _pipe_exists(pid: int) -> bool:
    """Cheap check: does the named pipe for this PID already exist?"""
    if not pid: return False
    try:
        h = open(f"{HOOK_PIPE_PREFIX}_{pid}", "rb"); h.close(); return True
    except Exception:
        return False


def _inject_hook(pid: int, timeout: float = 4.0) -> tuple:
    """
    Try to inject hooks/dinput_hook.dll into the target PID using the
    bundled injector.exe.  Returns (ok: bool, message: str).
    """
    if _pipe_exists(pid):
        return True, "Hook already injected."
    if not HOOK_DLL_PATH.exists():
        return False, (f"{HOOK_DLL_PATH.name} not found.\n"
                       f"Build it: cd {HOOK_DIR} && build.bat")
    if not HOOK_INJECTOR.exists():
        return False, (f"{HOOK_INJECTOR.name} not found.\n"
                       f"Build it: cd {HOOK_DIR} && build.bat")
    try:
        proc = subprocess.run(
            [str(HOOK_INJECTOR), str(pid), str(HOOK_DLL_PATH)],
            capture_output=True, text=True, timeout=timeout)
    except Exception as e:
        return False, f"injector failed to launch: {e}"
    if proc.returncode != 0:
        return False, f"injector exit {proc.returncode}: {proc.stdout.strip()} {proc.stderr.strip()}"
    # Give the DLL a moment to spin up its named-pipe server
    for _ in range(20):
        if _pipe_exists(pid):
            return True, "Hook injected and pipe is up."
        time.sleep(0.1)
    return False, "Injected but pipe never appeared — wrong bitness?"
...
class DetoursBackend(InputBackend):
    ...
    def __init__(self, target_pid: Optional[int] = None):
        ...
        # Auto-inject if needed — pipe takes care of the "already injected" case.
        if not _pipe_exists(target_pid):
        ...
        try:
            ...
            raise RuntimeError(f"Cannot open hook pipe {pipe_name}: {e}")
    ...
    def _send(self, line: str):
        try:
            ...
            print(f"[detours] write failed: {e}")
...
def create_backend(macro: "Macro") -> Optional[InputBackend]:
    ...
    Returns None if no usable backend could be created.
    """
    ...
    def _try_winmsg():
        h = _hwnd_for_macro()
        return BackgroundInjector(h) if h else None

    def _try_postmsg():
        h = _hwnd_for_macro()
        return PostMessageBackend(h) if h else None

    def _try_detours():
        pid = _pid_for_macro()
        if not pid: return None
        # In `auto` mode, only pick Detours if the pipe is already up — don't
        # surprise-inject DLLs.  When the user explicitly selects "detours"
        # the DetoursBackend constructor will auto-inject.
        if not _detours_pipe_ready(pid): return None
        return DetoursBackend(pid)

    builders = {
        "winmsg":       _try_winmsg,
        "postmsg":      _try_postmsg,
        "detours":      _try_detours,
    ...
    if choice in builders:
        try:    return builders[choice]()
        except Exception as e:
            print(f"[backend:{choice}] init failed: {e}")
            return None

    # AUTO mode: prefer the most specific backend that works for this macro.
    for fn in (_try_detours,   # hook DLL beats everything else when present
               _try_winmsg,
               _try_postmsg,
               lambda: SerialHIDBackend()    if SerialHIDBackend.available()    else None,
               lambda: InterceptionBackend() if InterceptionBackend.available() else None,
               lambda: PynputBackend()):
        try:
            b = fn()
            if b: return b
...
class PlayerThread(QThread):
    ...
    def run(self):
        ...
        # Tell the UI which backend we actually got (may differ from request
        # when "auto" was selected or the requested one is unavailable).
        self.mode_sig.emit(m.id, backend.name if backend else "none")
    ...
    def _fire(self, ev: dict, b: InputBackend):
        ...
        except Exception as e: print(f"[playback] {t}: {e}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Macro Recorder v{__version__}")
    ...
    def _build_topbar(self) -> QHBoxLayout:
        ...
        row.addStretch()
        title = QLabel(f"Macro Recorder v{__version__}")
    ...
    def _build_tab_events(self) -> QWidget:
        ...
        hh.resizeSection(0, 44); hh.resizeSection(1, 78); hh.resizeSection(2, 120)
    ...
    def _setup_tray(self):
        ...
        p.drawText(QRect(0, 0, 32, 32), Qt.AlignmentFlag.AlignCenter, "M"); p.end()
        ...
        self._tray.setContextMenu(menu); self._tray.setToolTip("Macro Recorder")
    ...
    def _on_backend_changed(self, idx: int):
        ...
        if name == "detours" and not HOOK_DLL_PATH.exists():
            self._set_status(
                f"Backend: Detours — DLL not built yet.  Click 'Hook DLL…' for build steps.",
                "#fab387")

    def _open_hook_dir(self):
        """Show hook-build status and open the hooks/dinput_hook/ folder."""
        dll_ok = HOOK_DLL_PATH.exists()
        inj_ok = HOOK_INJECTOR.exists()
        # Try to identify the current target's PID for context
        pid = None
        ...
        pipe_ok = _pipe_exists(pid) if pid else False
        msg = QMessageBox(self)
        ...
        lines = [
            ...
            "",
            f"dinput_hook.dll  : {'✓ found' if dll_ok else '✗ NOT BUILT'}",
            f"injector.exe     : {'✓ found' if inj_ok else '✗ NOT BUILT'}",
        ]
        if pid:
            lines += [
                "",
                f"Current target PID: {pid}",
                f"Pipe \\\\.\\pipe\\macro_hook_{pid}: "
                f"{'✓ injected' if pipe_ok else '✗ not injected (will auto-inject on Play)'}",
            ]
        if not (dll_ok and inj_ok):
            lines += [
                "",
                "<b>To build:</b>",
                "1. Install Visual Studio Build Tools (or full VS) with the 'Desktop C++' workload.",
                "2. Clone Microsoft Detours:  git clone https://github.com/microsoft/Detours",
                "3. cd Detours && nmake  (from a 'Developer Command Prompt for VS')",
                "4. set DETOURS=&lt;path&gt;",
                "5. Run build.bat inside hooks/dinput_hook/.",
            ]
    ...
    def _on_captured(self, ev: dict):
        ...
        self._ev_count.setText(f"{row + 1} events")
    ...
    def _stop_current(self):
        if self._current and self._current.id in self._players:
            self._players[self._current.id].stop()
    ...
    def _on_play_mode(self, mid: str, backend_name: str):
        ...
        if backend_name == "none":
            ...
            self._set_status(f"▶ '{label}' — API Hook (Detours) — fake state into DirectInput.", "#f5c2e7")
    ...
    def _on_play_stopped(self, mid: str):
        ...
        self._update_active_label()
        sound_play_stop(); speak("MACRO stopped")
    ...
    def _del_selected_events(self):
        ...
        for r in rows:
            if r < len(self._current.events): self._current.events.pop(r)
    ...
    def closeEvent(self, event):
        ...
        self._tray.showMessage("Macro Recorder",
